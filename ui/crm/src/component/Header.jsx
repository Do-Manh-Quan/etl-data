...
const Header = () => {
  ...
  return (
    <div className='bg-purple-500'>
      <div className='flex justify-between text-white container  py-4'>
        <div><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
        ...
        <div className='flex gap-6'>
          {
            PUBLIC_ROUTER.map((route) => <div key={route.key} className='cursor-pointer' onClick={() => {
              navigate(route.path)
            }}>
              {route.label}
            </div>)
